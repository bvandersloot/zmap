...
// zmap boilerplate
probe_module_t module_dns_raw;
static int num_ports;

const char dns_raw_default_domain[] = "www.google.com";
const uint16_t dns_raw_default_qtype = DNS_QTYPE_A;
 ...
 * with: dns_qtype (.h) dns_raw_qtype_strid_to_qtype (below) dns_raw_qtype_qtype_to_strid
 * (below, and dns_raw_setup_qtype_str_map())
 */
const char *dns_raw_qtype_strs[] = {"A",  "NS",  "CNAME", "SOA",   "PTR",
			    "MX", "TXT", "AAAA",  "RRSIG", "ALL"};
const int dns_raw_dns_raw_qtype_strs_len = 10;

const dns_qtype dns_raw_qtype_strid_to_qtype[] = {
...
int8_t dns_raw_qtype_qtype_to_strid[256] = {BAD_QTYPE_VAL};

void dns_raw_setup_qtype_str_map()
{
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_A] = 0;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_NS] = 1;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_CNAME] = 2;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_SOA] = 3;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_PTR] = 4;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_MX] = 5;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_TXT] = 6;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_AAAA] = 7;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_RRSIG] = 8;
	dns_raw_qtype_qtype_to_strid[DNS_QTYPE_ALL] = 9;
}

static uint16_t qtype_str_to_code(const char *str)
{
	for (int i = 0; i < dns_raw_dns_raw_qtype_strs_len; i++) {
		if (strcmp(dns_raw_qtype_strs[i], str) == 0)
			return dns_raw_qtype_strid_to_qtype[i];
	}
	return 0;
}
...
static int build_global_dns_packets(char *domains[], int num_domains)
{
	for (int i = 0; i < num_domains; i++) {
		...
		if (domains[i] != (char *)dns_raw_default_domain) {
			free(domains[i]);
		}
				      ...
				      fieldset_t *list)
{
	...
	if (qtype > MAX_QTYPE || dns_raw_qtype_qtype_to_strid[qtype] == BAD_QTYPE_VAL) {
		...
		fs_add_string(qfs, "qtype_str",
			      (char *)dns_raw_qtype_strs[dns_raw_qtype_qtype_to_strid[qtype]],
			      0);
	}
				    ...
				    fieldset_t *list)
{
	...
	if (type > MAX_QTYPE || dns_raw_qtype_qtype_to_strid[type] == BAD_QTYPE_VAL) {
		fs_add_string(afs, "type_str", (char *)BAD_QTYPE_STR, 0);
	} else {
		// I've written worse things than this 3rd arg. But I want to be
		// fast.
		fs_add_string(afs, "type_str",
			      (char *)dns_raw_qtype_strs[dns_raw_qtype_qtype_to_strid[type]],
			      0);
	}
...
static int dns_global_initialize(struct state_conf *conf)
{
	...
	for (int i = 0; i < num_questions; i++) {
		domains[i] = (char *)dns_raw_default_domain;
		qtypes[i] = dns_raw_default_qtype;
	}

	// This is zmap boilerplate. Why do I have to write this?
	num_ports = conf->source_port_last - conf->source_port_first + 1;
	udp_set_num_ports(num_ports);
	dns_raw_setup_qtype_str_map();
...
int dns_raw_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
		       UNUSED port_h_t dst_port, UNUSED void **arg_ptr)
{
	...
	module_dns_raw.packet_length = sizeof(struct ether_header) +
				   sizeof(struct ip) + sizeof(struct udphdr) +
				   dns_packet_lens[0];
	assert(module_dns_raw.packet_length <= MAX_PACKET_SIZE);
...
int dns_raw_make_packet(void *buf, size_t *buf_len, ipaddr_n_t src_ip,
...
void dns_raw_print_packet(FILE *fp, void *packet)
{
...
int dns_raw_validate_packet(const struct ip *ip_hdr, uint32_t len, uint32_t *src_ip,
			uint32_t *validation)
{
...
void dns_raw_process_packet(const u_char *packet, uint32_t len, fieldset_t *fs,
			uint32_t *validation)
{
...
probe_module_t module_dns_raw = {
    ...
    .port_args = 1,
    .thread_initialize = &dns_raw_init_perthread,
    .global_initialize = &dns_global_initialize,
    .make_packet = &dns_raw_make_packet,
    .print_packet = &dns_raw_print_packet,
    .validate_packet = &dns_raw_validate_packet,
    .process_packet = &dns_raw_process_packet,
